...
```java
//以下代码都可以data-view项目内我写过的代码结合起来一起看
...
这是因为SpringSecurity会自己去做登录的认证和权限的校验，靠的就是我们放在SpringSecurity上下文中的`SecurityContextHolder.getContext().setAuthentication(authentication);`，没有拿到authentication，放行了，SpringSecurity还是会走到认证和校验，这个时候就会发现没有登录没有权限。
...
- **控制token过期时间？如果用户在token过期前1秒还在操作，下1秒就需要重新登录，肯定不好（在data-view项目中，此处token过期时间的处理我完全放在了后端，前端不需要任何逻辑处理）**
...
##### 3.2 解决(不是很理解，jwt也有过期时间？？没看到哪里设置)
